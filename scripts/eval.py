...
def compute_metrics(translations, references, metrics, bleu_tokenize):
    results = []
    if "bleu" in metrics:
        bleu_metric = sacrebleu.metrics.BLEU(tokenize=bleu_tokenize)
        bleu_score = bleu_metric.corpus_score(translations, references)
        results.append(("BLEU", bleu_score.score, bleu_metric.get_signature()))
    if "chrf" in metrics:
        chrf_metric = sacrebleu.metrics.CHRF()
        chrf_score = chrf_metric.corpus_score(translations, references)
        results.append(("chrF", chrf_score.score, chrf_metric.get_signature()))
    return results
